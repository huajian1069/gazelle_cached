...
class GazeDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            return img, bbox_norm, gazex_norm, gazey_norm, torch.tensor(inout), height, width, heatmap, img_data['path']
        else:
            return img, bbox_norm, gazex_norm, gazey_norm, torch.tensor(inout), height, width, img_data['path']
